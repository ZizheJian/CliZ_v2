#ifndef __DC_4D_CUBIC_DATA_SET_MAP_MASK_CPP__
#define __DC_4D_CUBIC_DATA_SET_MAP_MASK_CPP__
...
{
	template<typename T>
	void task_c<T>::DC_4D_cubic_data_set_map_mask()
	{
		...
		if (mask_data[pos2horiz_mapping[0]]!=0)
		{
				...
				printf("Error: DC_4D_linear_data_set_map_mask shouldn't be called during decompression.\n");
				exit(0);
			#endif
			quant_bin_pos++;
		}
				...
				for (long long i1=0;i1<mx[1];i1+=stride)
					for (long long i3=0;i3<mx[3];i3+=2*stride)
					{
						...
						for (long long i2=stride;i2<mx[2];i2+=2*stride)
						{
							...
							if (i2+3*stride<mx[2])
								if (i2-3*stride>=0)
									pred=cubic_fitting_ddpdd_mask(pos+i2*weight[2],stride*weight[2]);
								else
									pred=quadratic_fitting_ddpd_mask(pos+i2*weight[2],-stride*weight[2]);
							else
								if (i2+stride<mx[2])
									if (i2-3*stride>=0)
										pred=quadratic_fitting_ddpd_mask(pos+i2*weight[2],stride*weight[2]);
									...
									if (i2-5*stride>=0)
										pred=quadratic_fitting_dddp_mask(pos+i2*weight[2],stride*weight[2]);
									else
										if (i2-3*stride>=0)
				...
				for (long long i1=0;i1<mx[1];i1+=stride)
					for (long long i2=0;i2<mx[2];i2+=stride)
					{
						...
						for (long long i3=stride;i3<mx[3];i3+=2*stride)
						{
							...
							if (i3+3*stride<mx[3])
								if (i3-3*stride>=0)
									pred=cubic_fitting_ddpdd_mask(pos+i3*weight[3],stride*weight[3]);
								else
									pred=quadratic_fitting_ddpd_mask(pos+i3*weight[3],-stride*weight[3]);
							else
								if (i3+stride<mx[3])
									if (i3-3*stride>=0)
										pred=quadratic_fitting_ddpd_mask(pos+i3*weight[3],stride*weight[3]);
									...
									if (i3-5*stride>=0)
										pred=quadratic_fitting_dddp_mask(pos+i3*weight[3],stride*weight[3]);
									else
										if (i3-3*stride>=0)
