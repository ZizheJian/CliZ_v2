#ifndef __DC_3D_LINEAR_DATA_USE_MAP_CPP__
#define __DC_3D_LINEAR_DATA_USE_MAP_CPP__
...
{
	template<typename T>
	void task_c<T>::DC_3D_linear_data_use_map()
	{
						...
						for (long long i1=(b1b?b1b+2*stride:0);i1<=b1e;i1+=2*stride)
							for (long long i2=(b2b?b2b+2*stride:0);i2<=b2e;i2+=2*stride)
							{
								...
								for (long long i0=b0b+stride;i0<=b0e;i0+=2*stride)
								{
									...
									#endif
									#ifdef JOB_TYPE_DECOMPRESS
										printf("Error: DC_3D_linear_data_use_map shouldn't be called during decompression.\n");
										exit(0);
									#endif
									quant_bin_pos++;
								}
