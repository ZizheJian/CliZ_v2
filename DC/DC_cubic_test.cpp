...
namespace cliz
{
	...
	void task_c<T>::DC_cubic_test()
	{
		...
		for (long long test_id=0;test_id<test_num;test_id++)
		{
			...
			{
				data[5]=data_backup[0];
				quant_bin[test_id]=quantize(5,0);
				continue;
			}
			...
			{
				if (right_data==0)
					pred=constant_fitting_dp(5,1);
				if (right_data==1)
					pred=linear_fitting_dpd(5,1);
				if (right_data==2)
					pred=quadratic_fitting_ddpd(5,-1);
			}
			if (left_data==2)
			{
				if (right_data==0)
					pred=linear_fitting_ddp(5,1);
				if (right_data==1)
					pred=quadratic_fitting_ddpd(5,1);
				if (right_data==2)
					pred=cubic_fitting_ddpdd(5,1);
			}
			if (left_data==3)
			{
				if (right_data==0)
					pred=quadratic_fitting_dddp(5,1);
			}
			quant_bin[test_id]=quantize(5,pred);
		}
		delete_data(data);
		data=data_backup;
		delete_data(i1);
