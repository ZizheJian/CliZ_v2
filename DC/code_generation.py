for dimension_num in [1,2,3,4]:
	for fitting_function in ["linear","cubic"]:
		...
		file.write(f"#ifndef __DC_{dimension_num}D_{fitting_function.upper()}_CPP__\n")
		file.write(f"#define __DC_{dimension_num}D_{fitting_function.upper()}_CPP__\n")
		file.write("\n")
		...
		file.write(f"	void task_c<T>::DC_{dimension_num}D_linear()\n")
		file.write("	{\n")
		...
		file.write("			quant_bin[quant_bin_pos]=quantize(0,0);\n")
		file.write("		#endif\n")
		file.write("		#ifdef JOB_TYPE_DECOMPRESS\n")
		file.write("			data[0]=dequantize(quant_bin_pos,0);\n")
		file.write("		#endif\n")
		...
		for direction in range(dimension_num):
			...
			file.write("	"*(tabnum+dimension_num+1)+f"pred=linear_fitting_dpd(pos+i{direction}*weight[{direction}],stride*weight[{direction}]);\n")
			file.write("	"*(tabnum+dimension_num)+"else\n")
			file.write("	"*(tabnum+dimension_num+1)+f"if (i{direction}-3*stride>=b{direction}b)\n")
			file.write("	"*(tabnum+dimension_num+2)+f"pred=linear_fitting_ddp(pos+i{direction}*weight[{direction}],stride*weight[{direction}]);\n")
			file.write("	"*(tabnum+dimension_num+1)+"else\n")
			file.write("	"*(tabnum+dimension_num+2)+f"pred=linear_fitting_dp(pos+i{direction}*weight[{direction}],stride*weight[{direction}]);\n")
			file.write("	"*(tabnum+dimension_num)+f"#ifdef JOB_TYPE_COMPRESS\n")
			file.write("	"*(tabnum+dimension_num+1)+f"quant_bin[quant_bin_pos]=quantize(pos+i{direction}*weight[{direction}],pred);\n")
			file.write("	"*(tabnum+dimension_num)+f"#endif\n")
			file.write("	"*(tabnum+dimension_num)+f"#ifdef JOB_TYPE_DECOMPRESS\n")
			file.write("	"*(tabnum+dimension_num+1)+f"data[pos+i{direction}*weight[{direction}]]=dequantize(quant_bin_pos,pred);\n")
			file.write("	"*(tabnum+dimension_num)+f"#endif\n")
