...
namespace cliz
{
	...
	void task_c<T>::choose_method()
	{
		//TODO: i32
			...
			new_data(best_decompress_function,FUNC_NAME_LENGTH);
			new_data(best_fitting_function,FUNC_NAME_LENGTH);
			strcpy(best_fitting_function,"cubic");
			copy_iterator(best_it1,it1);
			if (map_file_mode==NULL)
				if (mask_file_path==NULL)
					strcpy(best_decompress_function,"decompress");
				else
					strcpy(best_decompress_function,"decompress_mask");
			else
				if (mask_file_path==NULL)
					strcpy(best_decompress_function,"decompress_use_map");
				else
					strcpy(best_decompress_function,"decompress_use_map_mask");
		#endif
	}
}
