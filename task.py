def generate_command_compress(job_type,input_path,output_path=None,config=None,map=None,mask_path=None,dimension=None,data_type=None,err=None,debug=False):
	...
	if err!=None:
		if type(err)!=list:
			...
			if len(err)==1:
				...
				command+=" -err "+err[0]+str(err[1])
	command+=";"
	command+=f"calculateSSIM -f {source_path} {decompressed_path} "
	for i in range(len(dimension)-1,-1,-1):
		if type(dimension[i])==list:
			if len(dimension[i])==2:
